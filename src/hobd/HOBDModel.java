...
public class HOBDModel extends SpeciesTreeDistribution {
    ...
    public Input<RealParameter> medianInput = new Input<>("median",
            "Per-individual median.", Input.Validate.REQUIRED);
    ...
    RealParameter birthRate, superRate, deathRate, samplingRate, presentSamplingProb, timeOrigin, median;
    ...
    public void initAndValidate() {
        ...
        median = medianInput.get();

    }

    class ODEp0 implements FirstOrderDifferentialEquations {

        double lambda, mu, psi, rho, median, sup;

        public ODEp0(double lambda, double mu, double psi, double rho, double median, double sup) {
            this.lambda = lambda;
            this.mu = mu;
            this.psi = psi;
            this.rho = rho;
        ...
        public void computeDerivatives(double t, double[] y, double[] yDot)
            ...
            yDot[0] = -(lambda + mu + psi)*p0 + mu + lambda*p0*p0 + lambda* Math.exp(median*(p0-1))*p0;
        }
    }
    ...
    public double get_p0(double t) {
        ...
        ODEp0 p0equations = new ODEp0(birthRate.getValue(), deathRate.getValue(),
                samplingRate.getValue(), presentSamplingProb.getValue(), superRate.getValue(), median.getValue());
    ...
    public class ODEgep0 implements FirstOrderDifferentialEquations {
        ...
        public ODEgep0(double sup, double lambda, double mu, double psi, double rho, double median) {
            this.lambda = lambda;
            this.mu = mu;
            this.psi = psi;
            this.rho = rho;
        ...
        public void computeDerivatives(double t, double[] y, double[] yDot) {
            ...
            double p0Dot = -(lambda + mu + psi)*p0 + mu + lambda*p0*p0 + sup* Math.exp(median*(p0-1))*p0;
            double geDot = -(lambda + mu + psi)*ge + (2*lambda*p0*ge) +(sup*(median*p0+1)*Math.exp(median*(p0-1)));
...
//        TreeParser tree = new TreeParser("A:1.0;");
        ...
        HOBDModel bdmodelNumeric = new HOBDModel();
